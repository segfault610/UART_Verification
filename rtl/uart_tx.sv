module uart_tx #(
    ...
    always_ff @(posedge clk) begin
        case (state)
            ...
            START: begin
                tx_pin <= 0;
                if (count == WAIT_TIME - 1) begin count <= 0; state <= DATA; end
                else count <= count + 1;
            end
            DATA: begin
                ...
                if (count == WAIT_TIME - 1) begin
                    count <= 0;
                    if (bit_idx == 7) begin bit_idx <= 0; state <= PARITY; end
                    else bit_idx <= bit_idx + 1;
                end else count <= count + 1;
            end
            PARITY: begin
                tx_pin <= parity_bit;
                if (count == WAIT_TIME - 1) begin count <= 0; state <= STOP; end
                else count <= count + 1;
            end
            STOP: begin
                tx_pin <= 1;
                if (count == WAIT_TIME - 1) state <= IDLE;
                else count <= count + 1;
            end
        endcase
    end
